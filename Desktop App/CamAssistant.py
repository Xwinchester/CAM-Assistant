...
class ThreadExpert (BaseApp):
    ...
    def __update_chamfers(self, events=''):
        ...
        thread_data = next (
            ...
            None)
        if thread_data:
            chamfer_size = self.__caclulate_chamfer (thread_data["minor diameter"], thread_data['major diameter'],
                                                     user_chamfer_size)
...
class FolderFactory (BaseApp):
  ...
  """
    ...
    def __create_frame(self):
        ...
        create_folder_checkbox = Checkbutton (partFrame, text="Open Part Folder?", variable=self.CREATE_PART_FOLDER_VAR,
                                              font=(None, 16))
...
class Position:
    """
    stores position data for Cycle
    """
...
class Cycle:
    ...
    WEB_POCKET = {'name': 'WEB / POCKET', 'comment': ''}


class CycleEditor (Toplevel):
    """
    Edit menu for the probe builder
    """
    def __init__(self, parent, probebuilder, cycle_data, index):
        super ().__init__ (parent)
        # store info
        self.parent = parent
        self.probe_builder = probebuilder
        self.index = index
        self.cycle_data = cycle_data
        self.entry_vars = []
        # sets default font for gui
        self.font = (None, 15)

        #setup gui
        self.color_scheme = self.probe_builder.app.COLOR_SCHEME.value
        self.title (f"Probe Editor - {self.cycle_data['name'].title()}")
        self.geometry("")
        ...
        # Create a dictionary to map axis values to their corresponding index
        self.axis_index_mapping = {axis.value: index for index, axis in enumerate (Axis)}

    def __update_colors(self):
        self.config(bg=self.color_scheme['data_frame_background'])
    ...
    def __create_widgets(self):
        ...
        for label_text, entry_text in self.cycle_data.items():
            ...
            if isinstance(entry_text, Position):
                ...
                self.__create_regular_entry(label_text, entry_text, row)
                row += 1

        enter_button = Button(self, text="Enter", command=self.__on_enter, font=self.font)
        enter_button.grid(row=row, column=0, columnspan=3, padx=10, pady=10)

    def __create_widget(self, label_text, entry_widget, row):
        label = self.__create_label (label_text.title ())
        label.grid (row=row, column=0, padx=10, pady=5, sticky="e")
        entry_widget.grid (row=row, column=1, padx=10, pady=5, sticky="w")
        self.entry_vars.append (entry_widget)

    def __create_label(self, text):
        lbl = Label(self, text=text.title(), font=self.font)
        lbl.config(bg=self.color_scheme['data_frame_background'], fg=self.color_scheme['data_frame_font'])
        return lbl

    def __create_regular_entry(self, label_text, entry_text, row):
        entry_var = StringVar (value=entry_text)
        entry = Entry (self, textvariable=entry_var, font=self.font)
        self.__create_widget (label_text, entry, row)

    def __create_axis_entry(self, label_text, entry_text, row):
        selected_option = StringVar (value=entry_text)
        options = self.__get_all_axis_options ()
        option_menu = OptionMenu (self, selected_option, *options)
        self.__create_widget (label_text, option_menu, row)

    def __create_position_entries(self, entry_text, row):
        position = entry_text
        var_list = [DoubleVar (value=position.x), DoubleVar (value=position.y), DoubleVar (value=position.z)]
        entry_list = [Entry (self, textvariable=var, font=self.font) for var in var_list]
        labels = ["X", "Y", "Z"]

        for label, entry in zip (labels, entry_list):
            self.__create_widget (label, entry, row)
            row += 1

        self.entry_vars.append (var_list)

    def __get_all_axis_options(self):
        return [member.value for member in Axis]

    def __get_selected_axis_index(self, axis):
        axis_value = axis.get ()
        return self.axis_index_mapping[axis_value]
...
class ProbeBuilder (BaseApp):
    ...
    def __setup_bindings(self):
        """
        sets up the right click function to show the menu
        :return:
        """
    ...
    def __show_context_menu(self, event):
        """
        function to show the right click menu on the item you right clicked on
        :param event:
        :return:
        """
    ...
    def __edit_selected_item(self):
        """
        function to edit the selected option from the treeview, function comes from the right click menu
        :return:
        """
        ...
        if selected_item:
            ...
            editor = CycleEditor (self.app.APP, self,  self.cycles[item_index], item_index)

    def __remove_selected_item(self):
        """
        function to remove the select edoption from the treeview, function comes from the right click menu
        :return:
        """
        selected_item = self.tree.selection ()
        if selected_item:
    ...
    def __setup_right_click_menu(self):
        """
        sets up the right click menu, contains the following
        edit: edit the selected cycle
        delete: removes the selected option
        :return:
        """
    ...
    def __setup_buttons(self):
        """
        sets up the buttons to add each of the probe cycles
        :return: null
        """
    ...
    def __update_treeview(self):
        """
        updates the treeview with all the data inside the self.cycles
        :return:
        """
        self.tree.delete (*self.tree.get_children ())
        for cycle in self.cycles:
            # sets the text to the probe cycle type if comment doesnt equal anything
            text = f"{cycle['name']} - {cycle['comment']}" if cycle['comment'] else cycle['name']
            self.tree.insert ("", "end", values=(text,))
        self.__update_tags ()

    def __update_tags(self):
        """
        updates all of the tags, this way we can have differnt colors to view each row easier
        :return:
        """
        # Define the tag for even rows
        self.tree.tag_configure ("even", background=self.colors['even'])
    ...
    def __add_single_pt(self):
        item = Cycle.SINGLE_PT()
        self.cycles.append (item)
        self.__update_treeview ()

    def __add_bore_boss(self):
        item = Cycle.BORE_BOSS
        self.cycles.append (item)
        self.__update_treeview ()

    def __add_web_pocket(self):
        item = Cycle.WEB_POCKET
        self.cycles.append (item)
        self.__update_treeview ()
...
class App:
    ...
    def __load_json(self):
        try:
            ...
            self.JSON_CODE_VAULT = self.JsonFileManager.load_json_file (
                os.path.join (self.SOURCE_PATH, "code_vault.json"))
    ...
    def reset_colors(self):
        ...
        for widget in self.DATA_frame.winfo_children ():
            ...
            if widget.widgetName == "text":
                widget.config (bg=self.COLOR_SCHEME.value['message_background'],
                               fg=self.COLOR_SCHEME.value['message_font'])
